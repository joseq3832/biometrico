...
<li class="" )>
    ...
    <script type="text/javascript">
        ...
        $('#materias').on('change', function(e) { 
            ...
            $.get('/lista_alumnos_paralelo/'+{{$ultimoPeriodo}}+'/'+codseccion+'/'+codparalelo, function(data) {

                $('#tabledata').empty();
                $('#tabledata').append("<thead><tr><th>Nº</th><th>NOMBRES</th><th style='text-align: center;'>CÉDULA</th></thead>");
                for(var i=0; i<data.length; i++){
                    var cont=i+1;
                    $('#tabledata').append("<tbody><tr><th>"+cont+"</th><th>"+data[i].apepersona+" "+data[i].nompersona+"</th><th style='text-align: right;'>"+data[i].cedpersona+"  </th><tbody>"); 
                }
                $('#box-footer').empty();
                $('#box-footer').append("<br><a href='listaParaleloDocentepdf/"+{{$ultimoPeriodo}}+"/"+codseccion+"/"+codparalelo+"'><button type='button' class='btn btn-primary' onclick='ver()'>Imprimir</button></a>");
